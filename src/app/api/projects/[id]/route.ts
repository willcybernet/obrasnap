  ...
  try {
    ...
    if (updates && updates.length > 0) {
      ...
      const { data: photos, error: photosError } = await supabase
        .from('photos')
        .select('storage_path')
        .in('update_id', updates.map((update: { id: string }) => update.id))
      ...
      if (photos && photos.length > 0) {
        const storagePaths = photos.map((photo: { storage_path: string }) => photo.storage_path)
    ...
    if (updates && updates.length > 0) {
      const { error: updatesDeleteError } = await supabase
        .from('updates')
        .delete()
        .in('id', updates.map((update: { id: string }) => update.id))
