...
export default function ProjectPage() {
  ...
  const loadProject = useCallback(async () => {
    try {
      ...
      const orderedStages = [...(data.stages || [])].sort(
        (a: Stage, b: Stage) => a.order_index - b.order_index
      )
  ...
  const latestPhoto = project?.cover_image_url || project?.updates.flatMap((u) => u.photos || [])[0]?.storage_url
  ...
  const filteredUpdates = filterStage
    ...
    : project?.updates || []
  const stageNames = project?.stages.reduce((acc: Record<string, string>, s) => {
    acc[s.id] = s.name
    return acc
  }, {})
  /* ------------------- HELPERS ------------------- */
  ...
  const copyLink = () => {
    ...
    navigator.clipboard.writeText(
      `${window.location.origin}/obra/${project.public_slug}`
    )
    setCopied(true)
    setTimeout(() => setCopied(false), 2000)
  }
  /* ---- ONLY ONE DEFINITION OF handleUpdateStageDate --------------------------------- */
  const handleUpdateStageDate = async (
    stageId: string,
  ...
  return (
      ...
      <div className="mb-6 lg:mb-8 flex items-center justify-between">
        ...
        <div className="relative" ref={actionsRef}>
          ...
          {showActions && (
            <div
              className="absolute right-0 top-10 w-56 bg-surface-container-lowest rounded-xl shadow-lg border border-outline-variant/30 overflow-hidden z-[100]"
            >
              <button
          ...
          return (
            <div className="mb-6 p-4 bg-error-container text-on-error-container rounded-lg text-sm flex items-center gap-2">
              <span className="font-bold">
                Prazo vencido há {Math.abs(diffDays)} dias.
              </span>
        ...
        if (diffDays <= 7) {
          return (
            <div className="mb-6 p-4 bg-warning/10 text-warning-foreground rounded-lg text-sm flex items-center gap-2 border border-warning/20">
              ...
              <span>
                para o prazo estimado ({new Date(project.end_date).toLocaleDateString('pt-BR')})
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8">
        <div className="col-span-1 lg:col-span-8 space-y-6 lg:space-y-6">
          {/* PROJETO HEADERS */}
          <div>
          ...
          {/* PROJECT COVER & EDIT BUTTONS */}
          <div className="relative aspect-video w-full rounded-xl overflow-hidden bg-surface-container-low group">
            {latestPhoto ? (
              <img
                src={latestPhoto}
                alt="Foto do projeto"
            ...
            <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent flex flex-col justify-end p-6 lg:p-12">
              <span className="text-on-primary/70 text-xs lg:text-sm font-bold tracking-widest mb-1 lg:mb-2">
                PROJETO ATUAL
              </span>
              <h3 className="text-on-primary font-headline text-2xl lg:text-4xl font-medium italic">
                {project.name}
              </h3>
            </div>
            <button
                  ...
                  <button
                    ...
                    className={`shrink-0 px-3 py-1.5 rounded-full text-[10px] font-bold uppercase tracking-widest transition-colors ${
                      filterStage === stage.id
                        ...
                        : 'bg-surface-container text-outline hover:bg-surface-container-highest'
                    }`}
                  >
                    {stage.name}
                  </button>
            ...
            {filteredUpdates.map((update) => (
              <div key={update.id} className="bg-surface-container-low rounded-xl p-4 lg:p-6">
                {editingUpdate === update.id ? (
                  <div className="space-y-3">
                    ...
                    <select
                      ...
                      className="w-full bg-surface-container-low border-b-2 border-outline-variant focus:border-primary rounded-t-lg p-3 text-sm"
                    >
                  ...
                  <textarea
                    ...
                    className="w-full bg-surface-container-low border-none border-b-2 border-outline-variant focus:border-primary rounded-t-lg p-3 text-sm placeholder:text-outline/50 transition-all h-20 resize-none"
                    />({/* placeholder kept for UX */})<Input
                        type="text"
                        value={editUpdateNote}
                        onChange={(e) => setEditUpdateNote(e.target.value)}
                        className="w-full bg-surface-container-low border-b-2 border-outline-variant focus:border-primary rounded-t-lg p-3 text-sm placeholder:text-outline/50 transition-all h-20 resize-none"
                      />
                    </Input>
                    ...
                    <div className="flex gap-2">
                      ...
                      <Button
                        size="sm"
                        onClick={() => handleSaveUpdateEdit(updateId)}
                ...
                <>
                  ...
                  {update.note && (
                    <p className="text-on-surface-variant mb-3 lg:mb-4 text-sm lg:text-base">
                      {update.note}
                    </p>
                  )}
                  {update.photos &&
                    update.photos.length > 0 && (
                      <div className="flex gap-2 lg:gap-3 overflow-x-auto pb-2">
                        {update.photos.map((photo: any, photoIdx: number) => (
                          <div
                            key={photo.id}
                            className="w-16 lg:w-20 h-16 lg:h-20 rounded-lg overflow-hidden flex-shrink-0 cursor-pointer hover:opacity-80 transition-opacity"
                            onClick={() => {
              ...
              {stageUpdates.filter((s) => s.updateCount > 0).length > 0 && (
                <div className="bg-surface-container-low rounded-xl p-4 lg:p-6 border border-outline-variant/10 transition-all duration-300 hover:shadow-soft animate-fade-in-up">
                  <h4 className="font-label text-[10px] uppercase tracking-widest text-outline font-semibold mb-3">
                    Registros por Etapa
                  </h4>
        ...
        <div className="col-span-1 lg:col-span-4 space-y-4 lg:space-y-6">
            ...
            <div className="space-y-4 lg:space-y-6">
              ...
              <div className="pt-3 lg:pt-4 grid grid-cols-2 gap-3 lg:gap-4">
                <div className="bg-surface-container-highest p-3 lg:p-4 rounded-lg">
                  ...
                  <span className="font-headline text-xl lg:text-2xl font-bold">{project.stages.length}</span>
                </div>
                <div className="bg-surface-container-highest p-3 lg:p-4 rounded-lg">
                  <span className="block text-[10px] font-bold uppercase tracking-widest text-outline mb-1">Dias</span>
                  <span className="font-headline text-xl lg:text-2xl font-bold">
                    {project.start_date
                      ? Math.floor(
                        (Date.now() -
          ...
          {showRegister && (
            <div className="flex gap-3 mb-6">
              ...
              <Button disabled={selectedStages.length === 0} className="flex-1">
                Continuar
              </Button>
            </div>
          )}
          {registerStep === 1 && (
            <div className="space-y-4">
              <h2 className="font-headline text-lg lg:text-xl font-bold text-on-surface">
              ...
              <div className="grid grid-cols-3 gap-2 lg:gap-3">
                {[0, 1, 2, 3, 4, 5].map((i) => (
                  <div
                    ...
                    className="aspect-square bg-surface-container-low border-2 border-dashed border-outline-variant/30 rounded-xl flex flex-col items-center justify-center gap-2 cursor-pointer"
                  >
                    {photos[i] ? (
                      <img
          ...
          {registerStep === 3 && (
            <div className="space-y-4">
              ...
              <textarea
                ...
                className="w-full bg-surface-container-low border-none border-b-2 border-outline-variant focus:border-primary rounded-t-lg p-3 lg:p-4 text-sm placeholder:text-outline/50 transition-all h-24 lg:h-32 resize-none"
              />
            </div>
          )}
          {showRegister && (
          ...
          {showRegister && (
            <div className="p-6 bg-surface-container-low rounded-xl">
              <h4 className="font-label text-[10px] uppercase tracking-widest text-outline mb-2">
                Registros por Etapa
              </h4>
              <div className="space-y-2">
                {stageUpdates.filter((s) => s.updateCount > 0).map((stage) => (
